...
int try_to_release_page(struct page * page, int wait)
{
	if (!PageLocked(page))
		BUG();
	
	if (page->mapping && page->mapping->a_ops->releasepage)
		if (!page->mapping->a_ops->releasepage(page, wait))
			/* If we couldn't release metadata, don't even 
			 * bother trying to release buffers. */
			return 0;
