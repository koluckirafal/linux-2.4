...
static int __devinit probe_nic(struct pci_dev *pci_dev, const struct pci_device_id *id)
{
	...
	if (err) {
		printk(KERN_INFO "forcedeth: pci_enable_dev failed (%d) for device nForce2 Ethernet\n",
				err);
		goto out_free;
	}
	...
	for (i = 0; i < DEVICE_COUNT_RESOURCE; i++) {
		dprintk(KERN_DEBUG "NF2 Eth: resource %d start %p len %ld flags 0x%08lx.\n",
	...
	if (i == DEVICE_COUNT_RESOURCE) {
		printk(KERN_INFO "forcedeth: Couldn't find register window for device NF2 Eth.\n");
		goto out_relreg;
	}
	...
	if (!is_valid_ether_addr(dev->dev_addr)) {
		 ...
		 */
		printk(KERN_ERR "NF2 Eth: Invalid Mac address detected: %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	dprintk(KERN_DEBUG "NF2 Eth: MAC Address %02x:%02x:%02x:%02x:%02x:%02x\n",
			dev->dev_addr[0], dev->dev_addr[1], dev->dev_addr[2],
			dev->dev_addr[3], dev->dev_addr[4], dev->dev_addr[5]);
	...
	printk(KERN_INFO "%s: forcedeth.c: subsystem: %05x:%04x bound to NF2 Eth\n",
