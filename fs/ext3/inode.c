						...
						struct buffer_head *bh))
{
	...
	head = page->buffers;
	J_ASSERT(head != NULL);
	blocksize = page->mapping->host->i_sb->s_blocksize;
...
static void ext3_abort_write(struct file *file, struct page *page)
{
	handle_t *handle = ext3_journal_current_handle();
	struct inode *inode = page->mapping->host;
...
static int ext3_bmap(struct address_space *mapping, long block)
{
	...
	if (test_and_clear_bit(EXT3_STATE_JDATA, &inode->u.ext3_i.i_state)) {
		 ...
		 * everything they get.
		 */
		if (journal_flush(EXT3_JOURNAL(inode)))
			return 0;
	}
...
struct address_space_operations ext3_aops = {
	...
	abort_write:	ext3_abort_write,	/* BKL not held.  We take it */
		...
		u32 *first, u32 *last)
{
	 ...
	 */
	inode->i_blocks -= count * blocksize / 512;
#endif
}
